...
export default function useGsapAnimations() {
  ...
  useEffect(() => {
    ...
    // Load GSAP and plugins only on client side
    const loadGsap = async () => {
      if (typeof window !== "undefined") {
        const gsapModule = await import("gsap");
        const { CustomEase, SplitText } = await import("gsap/all");
        ...
        document.fonts.ready.then(() => {
          ...
          initializeAnimation(gsap, SplitText, ScrambleTextPlugin);
        });
      }
    };
  ...
  function initializeAnimation(gsap, SplitText, ScrambleTextPlugin) {
    ...
    // Set original text for each item
    ...
    // Add odd/even classes to type lines
    ...
    textRows.forEach((row, index) => {
      ...
      // Add this block after creating the SplitText:
      if (textElement.classList.contains("gradient-text")) {
        splitTexts[rowId].chars.forEach((char) => {
          ...
          // Also apply to inner spans if they exist
    ...
    // Initial animation for text rows
    ...
    // MISSING FUNCTION 1: initializeParallax
    ...
    // Function to force reset kinetic animation
    ...
    // Function to start kinetic animation
    ...
    // Function to fade out kinetic animation
    ...
    // Function to create text reveal animation
    ...
    // Function to reset background text with animation
    ...
    // MISSING FUNCTION 2: activateRow
    ...
    // Function to transition between rows
    function transitionBetweenRows(fromRow, toRow) {
      ...
      const toTextElement = toRow.querySelector(".text-content");
      if (!toTextElement) {
        state.transitionInProgress = false;
        return;
      }

      const toText = toTextElement.dataset.text;
      if (!splitTexts[toRowId]) {
        state.transitionInProgress = false;
        return;
      }
    ...
    // Add random text scramble effect
    ...
    // Add background text subtle animations
    backgroundTextItems.forEach((item, index) => {
    ...
    // Add stronger CSS rules to ensure kinetic type is visible
